...
Versão: GROK 4 + TENSÃO PSÍQUICA + DESENVOLVIMENTO DO AGENTE (SQLite)
"""
...
# ✅ NOVO: Import correto do módulo SQLite
import agent_development_db as dev_db

# Carregar variáveis de ambiente
load_dotenv()

# ✅ NOVO: Inicializar banco de dados SQLite
...
class InteractionMemory:
    ...
    detected_conflicts: List[ArchetypeConflict]
...
# ===============================================
# MÓDULO DE MEMÓRIA HÍBRIDO (ChromaDB + SQLite)
# ===============================================

class MemoryModule:
    """Módulo HÍBRIDO: ChromaDB (semântico) + SQLite (desenvolvimento)"""
    
    def __init__(self, persist_directory: str = "./chroma_db"):
        """Inicializa o módulo de memória híbrido"""
        self.persist_directory = persist_directory
        self.embeddings = OpenAIEmbeddings()
        
        # ChromaDB: Apenas memórias semânticas
        ...
        self._debug_log("Sistema híbrido: ChromaDB (semântico) + SQLite (desenvolvimento)")
...
"""
    ...
    async def store_memory(self, memory: InteractionMemory):
        """Armazena memória no ChromaDB E atualiza desenvolvimento no SQLite"""
...
class CentralOrchestrator:
    """Orquestrador que usa GROK 4 + DETECTA E EXPRESSA conflitos internos + RASTREIA DESENVOLVIMENTO"""
    ...
    def __init__(self):
        ...
        print("🧠 ORQUESTRADOR COM CONFLITO INTERNO + DESENVOLVIMENTO ATIVADO")
        log_capture.add_log("SISTEMA HÍBRIDO: ChromaDB + SQLite ATIVO", "🧠 SYSTEM")
        self.logger.info("Sistema com conflitos internos + rastreamento de desenvolvimento")
...
Sua TENDÊNCIA: Você prefere ACOLHER, VALIDAR, CONECTAR. Você busca proximidade emocional e empatia profunda."""
    ...
    async def reactive_flow(self, user_id: str, user_input: str, session_id: str = None,
        ...
        try:
            ...
            # 6. ARMAZENAR MEMÓRIA (ChromaDB + SQLite)
...
st.set_page_config(
    page_title="Claude Jung v2.0 - Conflito Interno + Desenvolvimento",
    page_icon="🧠",
    layout="wide",
...
def init_session_state():
    ...
    if 'orchestrator' not in st.session_state:
        with st.spinner("🧠 Inicializando sistema híbrido (ChromaDB + SQLite)..."):
...
# ========================================
# ✅ NOVO: Função para mostrar histórico de desenvolvimento
# ========================================
...
def show_development_history():
        ...
        if milestones:
            for m in milestones:
                with st.container():
                    ...
                    with col1:
                        st.markdown(f"**{m['type'].replace('_', ' ').title()}**")
                        st.write(f"_{m['description']}_")
                    with col2:
                        st.metric("Fase", m['phase'])
                        st.caption(m['timestamp'])
                    st.markdown("---")
        else:
            st.info("🌱 Nenhum milestone alcançado ainda. Continue interagindo!")
        ...
        if conflicts:
            for c in conflicts:
                with st.container():
                    col1, col2 = st.columns([3, 1])
                    with col1:
                        st.markdown(f"**{c['winner'].title()}** ⚡ **{c['loser'].title()}**")
                        st.write(f"Resolução: {c['resolution'][:100]}...")
                        if c['message']:
                            with st.expander("Ver contexto"):
                                st.write(c['message'])
                    with col2:
                        st.metric("Tensão", f"{c['intensity']:.2f}")
                        st.caption(c['timestamp'])
                    st.markdown("---")
        else:
            st.info("😇 Nenhum conflito registrado. Os arquétipos estão em harmonia!")
    ...
    with tab3:
        ...
        with col1:
            st.metric("Fase Atual", f"{agent_state['phase']}/5")
            st.metric("Interações Totais", agent_state['total_interactions'])
        
        with col2:
            st.metric("Total de Conflitos", stats['total_conflicts'])
            st.metric("Arquétipo Dominante", stats['dominant_archetype'])
        
        with col3:
            st.metric("Milestones", stats['total_milestones'])
            st.metric("Vitórias Dominante", stats['dominant_wins'])
        
        st.markdown("---")
        
        # Barras de progresso
        st.subheader("Dimensões de Desenvolvimento")
        
        st.write("**Auto-consciência**")
        st.progress(agent_state['self_awareness_score'], text=f"{agent_state['self_awareness_score']:.0%}")
        
        st.write("**Complexidade Moral**")
        st.progress(agent_state['moral_complexity_score'], text=f"{agent_state['moral_complexity_score']:.0%}")
        
        st.write("**Profundidade Emocional**")
        st.progress(agent_state['emotional_depth_score'], text=f"{agent_state['emotional_depth_score']:.0%}")
        
        st.write("**Autonomia**")
        st.progress(agent_state['autonomy_score'], text=f"{agent_state['autonomy_score']:.0%}")
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.subheader("⚡ **CONFLITO INTERNO + DESENVOLVIMENTO**")
        ...
        if st.session_state.user_id:
            ...
            st.write(f"**Fase:** {phase_names.get(agent_state['phase'], 'N/A')} ({agent_state['phase']}/5)")
            
            st.progress(agent_state['self_awareness_score'], text=f"Auto-consciência: {agent_state['self_awareness_score']:.0%}")
            st.progress(agent_state['moral_complexity_score'], text=f"Complexidade Moral: {agent_state['moral_complexity_score']:.0%}")
            ...
            with col1:
                st.metric("Conflitos", stats['total_conflicts'])
            with col2:
                st.metric("Milestones", stats['total_milestones'])
            
            if st.button("📊 Ver Histórico Completo", use_container_width=True):
                st.session_state.show_dev_history = True
                st.rerun()
...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        if st.session_state.get('show_dev_history', False):
            show_development_history()
        else:
            st.title(f"💬 Conversa com {st.session_state.user_name.split()[0]}")
            st.caption("⚡ Sistema híbrido: ChromaDB (semântico) + SQLite (desenvolvimento)")
